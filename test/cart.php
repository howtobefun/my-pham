...
<body class="theme-dark">
  ...
  <script>
    ...
    document.getElementById('btn-checkout').addEventListener('click', ()=>{
      ...
      // Gọi API checkout (cần đăng nhập)
      apiMe().then(u=>{
        if (!u) { alert('Vui lòng đăng nhập trước khi thanh toán.'); return; }
        return apiCheckout(t.map(i=>({ id:i.id, quantity:Number(i.quantity||1) })));
      }).then(res=>{
        if (!res) return;
        alert('Đặt hàng thành công! Mã đơn: '+res.order_id+'\nTổng tiền: $'+Number(res.total).toFixed(2));
        cart.splice(0, cart.length); saveCart(); renderCart(); updateCartCount && updateCartCount();
      }).catch(err=>{
